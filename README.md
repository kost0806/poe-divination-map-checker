...
실측 가중치는 **카드가 드롭됐을 때 그 카드일 확률**이다. 지도 1판에 카드가 몇 번 드롭되는지는
알 수 없지만, 같은 지도에서 예지만 바꿔 끼우는 비교라 그 값은 모든 지도에 공통이다. 그래서 기준값으로
고정하고 절대 수익 단위를 없앤 **비교 지수**만 쓴다.

```
대상(m) = { 지도 m 전용 점술 카드 c : c의 드롭 레벨 ≤ 지도 m 지역 레벨 }

확률_c   = 가중치_c ÷ 전체 카드 가중치 합
드롭수_c = (보스 전용이면 보스 드롭 횟수, 아니면 기준값) × 확률_c
지표_m   = Σ 드롭수_c × 시세_c        ← 지도 간 순위 기준
회수_m   = 예지의 오브 시세 ÷ 지표_m   (작을수록 빨리 회수)
```

기준값은 모든 지도에 똑같이 곱해지므로 순위와 배율에는 영향이 없고, 지표의 절대 크기에는 의미가 없다.
보스 전용 카드만 드롭 횟수가 다른데, 보스는 판당 한 번만 잡으므로 기회가 적기 때문이다.
**기준 1000 · 보스 150** 이라는 기본값은 실제 관측 두 건(창살 지도 약제사 100~200판에 1장,
묘지 지도 천벌 2700판에 1장)을 동시에 맞춘 값이다(모델 예측 189판 / 2588판).

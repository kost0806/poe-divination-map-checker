...
후보풀_m = Σ 대상(m) 가중치 + 지역 제한 없는 카드들의 가중치(전역 풀)
확률_c   = 가중치_c ÷ 후보풀_m
...
회수_m   = 예지의 오브 시세 ÷ 지표_m   (작을수록 빨리 회수)`}</pre>
      ...
      <p>
        후보 풀은 <b>전역 풀 + 그 지도 전용 카드</b>로 잡는다. 지역 제한이 없는 카드는 어디서나 후보이므로
        분모에서 뺄 수 없다. 지도 전용 카드만으로 분모를 잡으면 전용 카드가 전부 희귀한 지도에서 확률이
        터무니없어진다(상점가 지도는 전용 4장 가중치 합이 34라, 그렇게 계산하면 형제가 보인다가 전체 카드
        드롭의 46.7%를 차지한다는 결과가 나온다). 창살 지도 약제사(가중치 8.1)를 예로 들면 —
      </p>
      <table>
        <thead>
        ...
        <tbody>
          ...
          <tr><td>지도 4장만 (쓰지 않음)</td><td>18,657</td><td>1/2,311판</td></tr>
        </tbody>
      </table>
      <p>
        <b>내부 정합성 점검.</b> 같은 창살 지도의 집필가는 가중치 12,258 로 약제사(8.1)의 1,513 배다.
        즉 약제사가 150판에 한 장이라면 집필가는 <b>판당 약 10장</b>이어야 한다. 실제로 그렇게 나오지
        않는다면 약제사 체감이 실제보다 후하거나, 스택된 덱 가중치가 지역 드롭에 그대로 적용되지 않는
        것이다. 어느 가정도 "100~200판에 한 장"이라는 체감과는 맞지 않는다. 그렇게 되려면 지도 1판에 지역 카드가
